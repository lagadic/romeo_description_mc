...
#  jointsId = map(r.jointIdByName, jointsName)
  return tasks.qp.JointsSelector(robots.mbs(), index, hl, jointsName)
...
def positionTask(robots, robot_index, bodyName, pos, stiff, w, ef=Vector3d.Zero(), jointsName=[]):
  posTask = tasks.qp.PositionTask(robots.mbs(), robot_index, bodyName, pos, ef)
  if len(jointsName) > 0:
...
def orientationTask(robots, robot_index, bodyName, ori, stiff, w, jointsName=[]):
  oriTask = tasks.qp.OrientationTask(robots.mbs(), robot_index, bodyName, ori)
  if len(jointsName) > 0:
...
#TODO: add a trajectory task version also
...
# Helper functions to get body position
def getBodyPosW(robot, link):
  return robot.mbc.bodyPosW[robot.bodyIndexByName(link)]